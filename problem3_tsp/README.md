...
**2.2.1 Option 1:** quick demo using the same preloaded 6-location network as Problem 1.
**2.2.1.1** Choose one of the sample locations as the depot IE: "Depot" (case sensitive)
**2.2.1.2** Enter the delivery stops separated by spaces IE: "WarehouseA CityCenter Mall", or type "ALL" to deliver to every other location
**2.2.1.3** The program prints the shortest road distance between every pair of chosen locations, shows the fittest routes of the random starting population, then prints a generation-by-generation table of the best and average distance as the population evolves. It finishes with the greedy baseline route, the final Genetic Algorithm route (with the roads driven for each leg), and a comparison table.

**2.2.2 Option 2:** the same city extended with four more delivery zones.
**2.2.2.1** Same input steps as Option 1.
**2.2.2.2** With only five stops in Option 1 there are just 12 possible tours, so the algorithm finds the best one almost immediately and the evolution is not visible. This network has nine stops and 20,160 possible tours, so the population visibly improves over the generations (66 km down to 56 km), which is the behaviour a heuristic is meant to demonstrate.

**2.2.3 Option 3:** tests the edge case where a delivery stop cannot be reached.
**2.2.3.1** Choose "Depot" as the depot.
**2.2.3.2** Enter "ALL" or include "IsolatedZone" in the delivery stops.
**2.2.3.3** Since "IsolatedZone" has no roads connecting it to anything, no delivery tour covering it exists. Instead of crashing or inventing a route, the program detects this and prints a message stating that no route exists between the two locations.

**2.2.4 Option 4:** build your own network, then find a delivery tour around it.
**2.2.4.1** Enter the number of locations, then name each one.
**2.2.4.2** Enter the number of roads, then for each one type FromLocation ToLocation Distance (e.g. Depot WarehouseA 4).
**2.2.4.3** Choose the depot and the delivery stops.
4.4 The program runs the Genetic Algorithm on the network you built and prints the same output as Option 1.

**2.2.5 Option 5:** run the sample network with your own Genetic Algorithm settings.
**2.2.5.1** Enter the population size, number of generations, mutation rate, tournament size, number of elite routes, and the random seed. Press Enter at any prompt to keep the default shown in brackets.
**2.2.5.2** Choose the depot and delivery stops as in Option 1.
**2.2.5.3** Useful for showing how the settings affect the result. Because the same seed always produces the same output, any run can be repeated exactly.
...
**5.1** Does not guarantee the optimal tour. This is expected for a heuristic, and the comparison table reports how far the result is from the optimum whenever the network is small enough to check.
**5.2** Results depend on the settings used. Too small a population or too few generations can end the search before the best route is found, which Option 5 lets you demonstrate.
**5.3** Brute-force verification is only run for 9 or fewer delivery stops, since checking every tour beyond that takes too long.
**5.4** Requires non-negative distances, because the Dijkstra step used to build the road distance matrix does not guarantee correct results with negative weights. Negative distances are rejected during custom input.
**5.5** Assumes roads are bidirectional (two-way), matching a typical city road network.
**5.6** Location names are case sensitive and cannot contain spaces (use CityCenter, not City Center).
**5.7** Does not visualize the graph; output is text-based only, as required by the assignment brief (no GUI needed).